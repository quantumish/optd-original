...
use optd_og_perfbench::benchmark::Benchmark;
use optd_og_perfbench::cardbench::Cardinfo;
use optd_og_perfbench::job::JobKitConfig;
use optd_og_perfbench::tpch::{TpchKitConfig, TPCH_KIT_POSTGRES};
use optd_og_perfbench::{cardbench, job, shell, tpch};
use prettytable::{format, Table};

#[derive(Parser)]
struct Cli {
    #[clap(long)]
    #[clap(default_value = "optd_og_perfbench_workspace")]
    #[clap(
...
enum Commands {
    Cardbench {
        ...
        #[clap(long)]
        #[clap(action)]
        #[clap(help = "Whether to use the cached optd_og stats/cache generated stats")]
        // This is an option because you want to make it true whenever you update the
        //   code for how stats are generated in optd_og, in order to not use cached stats.
        // I found that I almost always want to use the cache though, which is why the
        //   system will use the cache by default.
        rebuild_cached_optd_og_stats: bool,

        #[clap(long)]
        #[clap(action)]
        #[clap(help = "Whether to enable adaptivity for optd_og")]
        adaptive: bool,
