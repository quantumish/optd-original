...
use optd_og_datafusion_bridge::create_df_context;
use optd_og_datafusion_repr_adv_cost::adv_stats::stats::{
...
impl DatafusionDBMS {
    ...
    fn log_explain(&self, explains: &[Vec<String>]) {
        // row_cnt is exclusively in physical_plan after optd_og
        let physical_plan_after_optd_og_lines = explains
            .iter()
            .find(|explain| explain.first().unwrap() == "physical_plan after optd_og")
            .unwrap();
        let explain_str = physical_plan_after_optd_og_lines.join("\n");
    ...
    /// This is used to execute the query in order to load the true cardinalities back into optd_og
    ...
    /// Load the data into DataFusion without building the stats used by optd_og.
    /// Unlike Postgres, where both data and stats are used by the same program, for this class the
    ///   data is used by DataFusion while the stats are used by optd_og. That is why there are two
    ...
    /// Build the stats that optd_og's cost model uses, or get the stats from the cache.
    async fn get_benchmark_stats(
        ...
        if !self.rebuild_cached_stats && stats_cache_fpath.exists() {
            ...
            // When self.rebuild_cached_optd_og_stats is true, we *don't read* from the cache but we
