...
/// Get the path of the root "optd_og" repo directory
pub fn get_optd_og_root() -> io::Result<PathBuf> {
    ...
    assert!(
        url_string.contains("cmu-db/optd_og"),
        "You are in the repo with url_string={}. This was not recognized as the optd_og repo.",
        url_string
    );
...
/// are evaluated relative to the optd_og repo root.
pub fn parse_pathstr(pathstr: &str) -> io::Result<PathBuf> {
    let path = PathBuf::from(pathstr);
    let path = if path.is_relative() {
        get_optd_og_root()?.join(path)
    } else {
        path
    };
    Ok(path)
...
pub fn clonepull_repo<P: AsRef<Path>>(repo_url: &str, repo_dpath: P) -> io::Result<()> {
    ...
    // make sure to do this so that get_optd_og_root() doesn't break
    Ok(())
}
