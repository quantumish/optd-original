...
impl DatafusionDBMS {
    ...
    async fn new_inner_no_optd_og() -> Result<Self> {
        let ctx = DatafusionDBMS::new_session_ctx_no_optd_og().await?;
        Ok(Self { ctx })
    }

    pub async fn new_no_optd_og() -> Result<Self, DbError> {
        Ok(Self::new_inner_no_optd_og().await?)
    }
    ...
    /// Creates a new session context without optd_og
    async fn new_session_ctx_no_optd_og() -> Result<SessionContext> {
