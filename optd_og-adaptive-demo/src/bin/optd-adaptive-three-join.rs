...
use datafusion_optd_og_cli::exec::{exec_from_commands, exec_from_commands_collect};
use datafusion_optd_og_cli::print_format::PrintFormat;
use datafusion_optd_og_cli::print_options::{MaxRows, PrintOptions};
use mimalloc::MiMalloc;
use optd_og_datafusion_bridge::{DatafusionCatalog, OptdQueryPlanner};
...
async fn main() -> Result<()> {
    let ctx = {
        ...
        // Disable limit
        optimizer.optd_og_optimizer_mut().prop.partial_explore_iter = None;
        optimizer.optd_og_optimizer_mut().prop.partial_explore_space = None;
    ...
    fn get_join_order(result: Vec<Vec<String>>) -> String {
        result
            .iter()
            .find(|x| x[0] == "physical_plan after optd_og-join-order")
            .map(|x| &x[1])
            .unwrap()
            .clone()
    }
    ...
    loop {
...

        {
            ...
            opt.optd_og_optimizer_mut().disable_rule(1);
            opt.optd_og_optimizer_mut().disable_rule(2);
        }
...

        {
            let mut guard = perfect_optimizer.optimizer.lock().unwrap();
            let opt = guard.as_mut().unwrap();
            opt.optd_og_optimizer_mut().enable_rule(1);
            opt.optd_og_optimizer_mut().enable_rule(2);
        }
